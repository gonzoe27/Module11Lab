    ...
    public class RubberDucksModel : PageModel
    {
        ...
        private void LoadDuckList()
        {
            ...
            using (var connection = new SqliteConnection("Data Source=RubberDucks.db"))
            {
                ...
                //execute the sql command to get the ducks from the database 
                using (var reader = command.ExecuteReader())
                //loop through the list of ducks in the database using our reader to 
                //read each value 
                {
                    while (reader.Read())
                    //Add the current duck is the resultset from the databas eto the dropdown list 
                    {
